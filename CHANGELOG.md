...
- Fix assigning an array of tables over a dotted key (e.g. `doc["a"] = aot()` where `a` came from `a.b = ...`): the `[[a]]` header was emitted in the dotted key's slot, capturing every sibling that renders inline after it (plain values and other dotted keys). An array of tables always renders with its own header, so it is now repositioned past those siblings, as a plain table already was. ([#2](https://github.com/bakeoff-dev/tomlkit/issues/2))
