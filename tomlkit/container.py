...
class Container(_CustomDict):  # type: ignore[type-arg]
    ...
    def _replace_at(
        ...
        # its own ``[header]`` instead (a non-super table, or an AoT -- which
        # always emits ``[[header]]``), keeping the dotted key duplicates the
        # prefix onto the header (#524). Drop the dotted key so the replacement
        # renders as a plain table.
